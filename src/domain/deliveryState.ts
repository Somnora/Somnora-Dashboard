...
import { createActionOutcome } from './actionRuntime'
...
const allowedTransitions: Record<DeliveryStatus, DeliveryStatus[]> = {
  ...
  'delivered-phone': ['delivered-watch', 'in-progress', 'failed', 'cancelled', 'expired'],
...
export function transitionDelivery(
  ...
  return {
    ...
    status,
    updatedAt,
    outcome: createActionOutcome(status === 'idle' ? 'pending' : status, updatedAt),
...
export function updateDeliveryProgress(
  ...
  if (!Number.isInteger(count) || count < state.progressCount || count > state.progressTarget) {
    throw new Error(`Invalid progress count: ${count}`)
  }

  return {
    ...state,
    status: count === state.progressTarget ? 'completed' : 'in-progress',
    progressCount: count,
    updatedAt,
    outcome: count === state.progressTarget
      ? createActionOutcome('completed', updatedAt)
      : undefined,
  }
}
...
export function reconcileDeliverySnapshot(
  ...
  if (snapshot.progressCount < state.progressCount || snapshot.progressCount > state.progressTarget) {
    throw new Error(`Invalid progress count: ${snapshot.progressCount}`)
  }
  if (snapshot.status === 'completed' && snapshot.progressCount !== state.progressTarget) {
    throw new Error('Completion requires the prepared progress target')
  }
  if (
  ...
  return {
    ...
    outcome: createActionOutcome(
      snapshot.status === 'idle' ? 'pending' : snapshot.status,
      updatedAt,
    ),
  }
}
