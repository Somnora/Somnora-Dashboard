...
test('@visual capture the recordable dashboard fixtures', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Find an action that fits the person you are today.' })).toBeVisible()
  await capture(page, 'activity-studio-1440')
  await page.setViewportSize({ width: 1280, height: 800 })
  await capture(page, 'activity-studio-1280')
  await page.setViewportSize({ width: 1440, height: 900 })
  await page.getByRole('button', { name: 'Begin Six Line Story' }).click()
  await expect(page.getByRole('dialog')).toBeVisible()
  await capture(page, 'six-line-story-1440')
})
